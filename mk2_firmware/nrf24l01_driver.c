...
void nrf_tx_packet(uint8_t* packet) {
	...
	// WRITE 0x20
	SPI_tx_byte(0x20);
	//USART_SendString("tx_packet: ", 11);
	//USART_SendByteString(ret);
	// WRITE 0x72
	SPI_tx_byte(0x72);
	// CS HIGH
	PORTB |= (1 << SS_PIN);
...
void nrf_set_tx_address(uint8_t* address) {
	...
	SPI_tx_byte(0x30);
	_delay_us(200);
	//USART_SendString("nrf_set_tx status: ", 19);
	//USART_SendByteString(ret);
	// write address
	for (i = 0; i < 5; i++) {
...
void nrf_set_rx_address(uint8_t* address) {
	...
	SPI_tx_byte(0x2A);
	_delay_us(200);
	//USART_SendString("nrf_set_rx status: ", 19);
	//USART_SendByteString(ret);
	// write address
	for (i = 0; i < 5; i++) {
		SPI_tx_byte(address[i]);
...
void nrf_set_chan(uint8_t chan) {
	...
	// write 0x25
	SPI_tx_byte(0x25);
	//USART_SendString("nrf_set_chan status: ", 21);
	//USART_SendByteString(ret);
	// send chan
	SPI_tx_byte(chan);
	// SS high
